...
-- Try manually instering a position
Insert into Position(PositionDescription)
VALUES ('The IT guy who has to figure out why things are not working as expected')
-- The errror from running the line above was generated by the position table.
-- In the Position table the PositionDescription is set to varchar (50).
-- Meaning the data base will not accept more than 50 characters(Truncated).
SELECT MAX(PositionID) FROM Position
-- What will the Position ID Be After an insert?
